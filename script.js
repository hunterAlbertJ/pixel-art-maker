...
function save() {
  ...
  console.log(tempArr);
  localStorage.setItem("savedPainting", JSON.stringify(tempArr));
}
function load() {
  var savedPaining = JSON.parse(localStorage.savedPainting);
  for (var e = 0; e < savedPaining.length; e++) {
    document.getElementsByTagName("DIV")[e].style.backgroundColor =
      savedPaining[e];
  }
...
var resetButton = document.createElement("button");
resetButton.setAttribute("id", "load");
resetButton.style.height = "2.0%";
resetButton.style.width = "5.0%";
resetButton.style.float = "left";
resetButton.style.paddingBottom = "5.0%";
...
resetButton.innerHTML = "LOAD";
resetButton.addEventListener("click", load);
document.getElementById("foot").appendChild(resetButton);
